...
import { BookOpen, Trophy, ArrowUpRight, Clock, HelpCircle, Play, Lock } from 'lucide-react'
...
export default function CourseTabs({ 
  ...
  const handleAccessCheck = (e: React.MouseEvent, id: string, type: 'subject' | 'mock') => {
    ...
    toast.error("Access Restricted", {
      description: "You do not have access to this content. Please contact your administrator.",
      duration: 4000,
    })
  }
        ...
        <div className="grid gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
          {subjects.length === 0 ? <EmptyState icon={BookOpen} text="No subjects found." /> : 
            subjects.map((subject: any, index: number) => {
              ...
              const isLocked = !isAdmin && !enrolledSubjectIds.includes(subject.id)
              ...
              return (
                <Link 
                  ...
                  className={`group block relative ${isLocked ? 'cursor-not-allowed' : ''}`}
                >
                  <div className={`
                    ...
                    <div className="flex items-center gap-6">
                      <div className={`w-16 h-16 rounded-2xl flex items-center justify-center border-2 ${bgColor} ${borderColor} text-black font-black text-xl shadow-sm shrink-0`}>
                        {isLocked ? <Lock className="w-6 h-6" /> : subject.title.substring(0, 2).toUpperCase()}
                      </div>
                      <div>
                        <div className="flex items-center gap-2 mb-1">
                           ...
                           {isLocked && <span className="px-2 py-0.5 bg-gray-200 text-gray-500 text-[10px] font-bold uppercase rounded">Locked</span>}
                        </div>
        ...
        <div className="grid gap-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
          {examMocks.length === 0 ? <EmptyState icon={Trophy} text="No full mock tests generated yet." /> :
            examMocks.map((mock: any) => {
              ...
              return (
                <div key={mock.id} className={`group flex flex-col md:flex-row md:items-center justify-between p-6 rounded-3xl border-2 bg-white transition-all border-gray-100 hover:border-blue-200`}>
                  <div className="flex gap-5 items-start">
                    <div className={`w-12 h-12 rounded-2xl flex items-center justify-center border-2 shrink-0 bg-red-50 border-red-100 text-red-600`}>
                      {isLocked ? <Lock className="w-5 h-5" /> : <Trophy className="w-6 h-6" />}
                    </div>
                    <div>
                      <div className="flex items-center gap-2 mb-2">
                        <h3 className="text-xl font-black text-gray-900">{mock.title}</h3>
                        {isLocked && <span className="px-2 py-0.5 bg-gray-200 text-gray-500 text-[10px] font-bold uppercase rounded">Locked</span>}
                      </div>
