...
export default function ContentManager({ streams }: { streams: Stream[] }) {
  ...
  const openStreamModal = (mode: 'create' | 'edit', data?: Stream) => {
    if (mode === 'edit' && data) {
      ...
      setStreamForm({ title: '', description: '', icon: 'PenTool', color: '#CEFF1A', order: 1 })
    }
        ...
        <div className="bg-white rounded-3xl w-full max-w-5xl shadow-2xl overflow-hidden max-h-[90vh] flex flex-col md:flex-row">
          <div className="p-8 flex-1 overflow-y-auto">
            ...
            <form action={async (formData) => {
                ...
                if (isEdit) await updateStreamAction(formData)
                else await createStreamAction(formData)
                setModal(null)
            }} className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input name="title" value={streamForm.title} onChange={e => setStreamForm(p => ({ ...p, title: e.target.value }))} placeholder="e.g. Engineering" required className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-black outline-none" />
                </div>
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-1">Order</label>
                  <input name="order_index" type="number" value={streamForm.order} onChange={e => setStreamForm(p => ({ ...p, order: parseInt(e.target.value) }))} className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-black outline-none" />
                </div>
              </div>
              <div>
                <label className="block text-sm font-bold text-gray-700 mb-1">Description</label>
                <textarea name="description" rows={2} value={streamForm.description} onChange={e => setStreamForm(p => ({ ...p, description: e.target.value }))} className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-black outline-none" />
              </div>
              <div>
                ...
                <div className="flex gap-3">
                  ...
                  <button type="button" onClick={() => setIsPickerOpen(!isPickerOpen)} className="flex-1 px-4 py-3 bg-white border border-gray-200 rounded-xl text-left text-sm font-medium text-gray-700 hover:border-black transition-all flex items-center justify-between">
                    <span>{streamForm.icon}</span><span className="text-xs bg-black text-white px-2 py-1 rounded">Change</span>
                  </button>
                </div>
                {isPickerOpen && (
                  <div className="mt-2 p-4 bg-gray-50 border border-gray-100 rounded-xl">
                    <div className="mb-3 relative">
                       ...
                       <input type="text" placeholder="Search icons..." className="w-full pl-9 pr-3 py-2 text-sm rounded-lg border border-gray-200" onChange={(e) => setSearchQuery(e.target.value)} />
                    </div>
                    <div className="grid grid-cols-6 gap-2 max-h-40 overflow-y-auto">
                       {filteredIcons.map(icon => (
                         <button key={icon} type="button" onClick={() => { setStreamForm(p => ({...p, icon})); setIsPickerOpen(false) }} className={`p-2 rounded-lg flex items-center justify-center hover:bg-white hover:shadow-sm transition-all ${streamForm.icon === icon ? 'bg-black text-white' : 'text-gray-500'}`}>
              ...
              <div>
                ...
                <div className="flex items-center gap-4">
                  <input type="color" value={streamForm.color} onChange={e => setStreamForm(p => ({ ...p, color: e.target.value }))} className="h-10 w-16 p-1 rounded-lg border border-gray-200 cursor-pointer" />
          ...
          <div className="w-full md:w-80 bg-gray-50 p-8 border-l border-gray-100 flex flex-col items-center justify-center">
             ...
             <div className="w-full aspect-square rounded-4xl p-6 flex flex-col justify-between shadow-2xl transition-all duration-300" style={{ backgroundColor: streamForm.color }}>
                <div className="w-12 h-12 bg-black/10 rounded-2xl flex items-center justify-center backdrop-blur-sm">
                   <DynamicIcon name={streamForm.icon} className="w-6 h-6 text-gray-900" />
                </div>
                <div className="text-left">
                   <h3 className="text-3xl font-black text-gray-900 leading-none mb-2 wrap-break-words">{streamForm.title || 'Title'}</h3>
                   <p className="text-sm font-medium text-gray-900/60 line-clamp-3 leading-snug">{streamForm.description || 'Description will appear here...'}</p>
                </div>
             </div>
          </div>
        </div>
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm animate-in fade-in min-h-screen">
        <div className="bg-white rounded-3xl p-8 w-full max-w-md shadow-2xl">
          ...
          <form action={async (formData) => { await action(formData); setModal(null); }} className="space-y-4">
            ...
            <button type="submit" className="w-full bg-black text-white font-bold py-3 rounded-xl hover:bg-gray-800 transition-all mt-2">Save Changes</button>
          </form>
        </div>
      </div>
    )
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8 pb-20">
      ...
      <div className="flex items-center justify-between">
        <div><h1 className="text-3xl font-black text-gray-900 tracking-tight">Content Manager</h1><p className="text-gray-500">Manage Streams, Exams, and Subjects in one place.</p></div>
        <button onClick={() => openStreamModal('create')} className="bg-black text-white px-5 py-2.5 rounded-xl font-bold flex items-center gap-2 hover:bg-gray-800 transition-all shadow-lg"><Plus className="w-4 h-4" /> Add Stream</button>
      </div>
            ...
            <div key={stream.id} className="bg-white border border-gray-200 rounded-3xl overflow-hidden shadow-sm transition-all">
              <div className="flex items-center justify-between p-4 bg-gray-50/50 border-b border-gray-100 group">
                <button onClick={() => toggleStream(stream.id)} className="flex items-center gap-4 flex-1 text-left">
                  <div className={`w-12 h-12 rounded-xl flex items-center justify-center border-2 border-black/10 shrink-0 ${finalClass}`} style={finalStyle}>
                     <DynamicIcon name={stream.icon_key || 'Circle'} className="w-5 h-5 text-gray-900 opacity-70" />
                  </div>
                  <div>
                    <h3 className="text-lg font-bold text-gray-900 group-hover:text-blue-600 transition-colors">{stream.title}</h3>
                    <div className="flex items-center gap-2 text-sm text-gray-500 font-medium">
                      <span className="bg-gray-100 px-2 py-0.5 rounded text-xs uppercase tracking-wide">{stream.courses.length} Exams</span>
                      <span>•</span><span className="text-gray-400">{isStreamOpen ? <ChevronDown className="w-4 h-4" /> : <ChevronRight className="w-4 h-4" />}</span>
                    </div>
                  </div>
                </button>
                        ...
                        <div className="flex items-center justify-between p-3 bg-white hover:bg-gray-50 transition-colors cursor-pointer" onClick={() => toggleExam(exam.id)}>
                           <button className="flex items-center gap-3 flex-1 text-left pl-2">
                              ...
                              <div>
                                ...
                                <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wide">{subjectCount} Subjects</span>
                              </div>
                           </button>
