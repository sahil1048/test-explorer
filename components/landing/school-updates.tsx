...
const getValidImageUrl = (url: string) => {
  ...
  try {
    ...
    if (urlObj.hostname.includes('drive.google.com')) {
      ...
      if (fileId) {
        // The most robust way to embed Drive images right now (bypasses CORS & Cookie blocks)
...
export default async function SchoolUpdates({ school }: { school: any }) {
  ...
  const hasPrincipalSection = school.principal_message || school.principal_image;
  const principalImgSrc = getValidImageUrl(school.principal_image);
      ...
      {hasPrincipalSection && (
        <section className="container mx-auto px-4 sm:px-6 max-w-7xl pt-12 pb-10">
          <div className="bg-white rounded-[2rem] border border-gray-200 shadow-lg p-8 md:p-12 relative overflow-hidden">
            ...
            {/* Background Quote Icon for design */}
            <Quote className="absolute top-8 right-8 w-32 h-32 text-gray-50 opacity-50 rotate-180 pointer-events-none" />
            ...
            <div className="grid md:grid-cols-12 gap-10 items-center relative z-10">
              ...
              <div className="md:col-span-8 lg:col-span-9 space-y-6">
                <h3 className="text-3xl md:text-4xl font-black text-gray-900">A Message to Our Students</h3>
                <div className="relative">
                  <p className="text-lg text-gray-600 leading-relaxed relative z-10 whitespace-pre-line pl-4 border-l-4 border-blue-500 rounded-l">
                    {school.principal_message || "Welcome to our institution. Our goal is to inspire and empower each of you to reach your highest potential. Embrace the challenges ahead, stay curious, and remember that your education is the foundation of your future success. We are here to support you every step of the way."}
                  </p>
                </div>
              </div>
